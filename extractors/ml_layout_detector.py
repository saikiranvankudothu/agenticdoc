...
class DiTLayoutDetector:
    ...
    MODEL_ID   = "microsoft/dit-base-finetuned-doclaynet"
    SCORE_THR  = 0.60
    ...
    def _load(self):
        ...
        try:
            ...
            logger.info("DiT model loaded: %s (device=%s)", self.model_id, self._device)
        except Exception as e:
            raise RuntimeError(
    ...
    def detect(
        ...
        """Run DiT object detection on a rendered page image."""
        self._load()
        import torch  # type: ignore
        ...
        # Post-process: convert logits → boxes in [0,1] relative coords
        target_sizes = torch.tensor([page_image.size[::-1]])  # (H, W)
        ...
        logger.debug(
            "DiT: page %d → %d regions", page.page_index, len(regions)
        )
        return regions
